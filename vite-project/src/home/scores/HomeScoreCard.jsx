...
function HomeScoreCard(props) {
  ...
  return (
    ...
    <div style={{ backgroundColor: generateBackgroundColor() }} className="card work">
      <Link className="card-link" to={`/song/${props.mp3Id}`} key={props.mp3Id}>
        ...
        <div className="card-time">Top Score: {props.score} User: {props.username}</div>
        <p className="recent">{props.scoreTimestamp}</p>
      </Link>
    </div>
